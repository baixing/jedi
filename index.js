...
function run(jedi, source, model) {
	try {
...
app.get('/jedi', function(req, res){
	var src = req.query.template
	if (!src) return res.end('No template provided')
	try {
		var model = JSON.parse(req.query.model)
	} catch(e) {
		return res.end('query.model is not valid JSON: ' + req.query.model)
	}
	require('my').load('jedi', function(jedi){
		res.end(
			run(jedi, src, model)
